...
| `PI_SESSIONS_ROOT` | No | `~/.pi/agent/sessions` | Directory containing Pi session `.jsonl` files |
...
If your Pi data lives somewhere else, set `PI_SESSIONS_ROOT` to the directory that contains the session `.jsonl` files, for example `~/.omp/agent/sessions`.
